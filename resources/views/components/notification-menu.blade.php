...
<div class="relative" x-data="{ notificationsOpen: false, unreadCount: {{ (int) ($navbarUnreadCount ?? 0) }}, busyNotification: null, async markNotificationRead(id, event) {
    ...
    >
        ...
        <span x-show="unreadCount > 0" x-text="unreadCount > 9 ? '۹+' : unreadCount" class="absolute -top-1 -left-1 min-w-4 h-4 px-1 rounded-full bg-[#EF4444] text-white text-[8px] font-bold flex items-center justify-center ring-2 ring-[#071B33]"></span>
    </button>
            ...
            @forelse (($navbarNotifications ?? collect()) as $notification)
                <div data-notification-id="{{ $notification->id }}" class="flex items-center gap-2 px-4 py-3 border-b border-[#F1F5F9] last:border-0 hover:bg-[#F8FAFC] transition-colors {{ $notification->read_at ? '' : 'bg-[#F5F9FF]' }}">
                    ...
                    @if (! $notification->read_at)
                        <button type="button" data-read-button @click="markNotificationRead('{{ $notification->id }}', $event)" :disabled="busyNotification === '{{ $notification->id }}'" class="shrink-0 rounded-lg border border-[#D8D0C3] px-2 py-1.5 text-[9px] font-bold text-[#66716B] hover:border-[#18212B] hover:text-[#18212B] disabled:opacity-50">خوانده شد</button>
