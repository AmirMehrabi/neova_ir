...
@section('content')
    <h1 style="margin: 0 0 16px; font-size: 18px; font-weight: 700; color: #0b1b2b; line-height: 1.5;">
        {{ $actor }} وظیفه «{{ $task->title }}» را به‌روز کرد
    </h1>
    ...
    <p style="margin: 0 0 24px; font-size: 14px; color: #5f6b73; line-height: 1.7;">
        {{ $actor }} وظیفه «{{ $task->title }}» در پروژه «{{ $project->name }}» را تغییر داد.
        برای دیدن جزئیات جدید روی دکمه زیر کلیک کنید.
    </p>
    ...
    <a href="{{ $url }}" class="btn-primary" style="color: #ffffff;">مشاهده تغییرات →</a>
